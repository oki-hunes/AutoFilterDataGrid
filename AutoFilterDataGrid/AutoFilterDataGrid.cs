...
namespace BetterDataGrid
{
    ...
    public class AutoFilterDataGrid : DataGrid, INotifyPropertyChanged
    {
        ...
        protected void DeleteCellValue(DataGridCellInfo thisCell)
        {
            ...
            if (valueType.IsValueType && Nullable.GetUnderlyingType(valueType) == null && valueType.GetConstructor(Array.Empty<Type>()) == null && thisCell.Item.GetType().GetProperty(propertyPath.Path) != null && !IsNumericType(thisCell.Item.GetType().GetProperty(propertyPath.Path).PropertyType))
            {
